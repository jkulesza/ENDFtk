...
SCENARIO( "parsing a simple GENDF section" ) {
  GIVEN( "a string representation of a GendfSection" ) {
    WHEN( "there is a syntaxTree::GendfSection" ) {
      ...
      THEN( "the section can be parsed" ) {
        section::GendfType chunk = section.parse();

        // number of list records
        CHECK( chunk.lists().size() == 44 );

        // check temperatures
        CHECK( chunk.lists()[0].C1() == Approx(293.6) );
        CHECK( chunk.lists()[43].C1() == Approx(293.6) );

        // check cross sections
        CHECK( chunk.xs()[0] == Approx(14.69141) );
