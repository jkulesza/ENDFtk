...
SCENARIO( "testing GENDF tape read" ) {
  GIVEN( "a string representation of a GendfTape" ) {
    WHEN( "duplicate section is present" ){

      std::string buffer = headerString() + mfd3mt2() + mfd3mt2() +
                           validMEND() + validTEND();

      THEN( "the object can be constructed and queried." ) {

        syntaxTree::GendfTape tape( buffer );
        CHECK(tape.size() == 1);
        CHECK(tape.hasMaterialNumber(9228));
...
std::string headerString() {
  return
    "ENDF, TENDF -> UNRESR, GROUPR -> GENDF                               0 0  0    0\n"
