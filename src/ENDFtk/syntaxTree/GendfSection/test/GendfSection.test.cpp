...
SCENARIO( "testing creation of GENDF section object" ) {
  GIVEN( "a string representation of a GendfSection" ) {
    WHEN( "a valid section is given" ){
      ...
      AND_THEN( "the fields are correctly populated" ) {
        CHECK( section.fileNo() == 1 );
        CHECK( section.sectionNo() == 451 );
      }

    }
    WHEN( "an invalid SEND is given" ) {
      std::string buffer = sampleSection() + invalidSEND();
      auto begin = buffer.begin();
      auto position = begin;
      auto end = buffer.end();
      long lineNo = 0;
